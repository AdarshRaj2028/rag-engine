...
# Document Cleanup Methods
    ...
    def get_oldest_documents(self, limit: int = 1) -> List[Dict]:
        """
        Get the oldest documents ordered by upload timestamp
        
        Args:
            limit: Maximum number of documents to return
        
        Returns:
            List of document dictionaries ordered from oldest to newest
        """
        try:
            self.cursor.execute("""
                SELECT 
            ...
            logger.info(f"Retrieved {len(documents)} oldest documents")
            return documents
        except sqlite3.Error as e:
            logger.error(f"Error getting oldest documents: {e}")
            return []

    def delete_document_and_dependencies(self, document_id: str) -> bool:
        """
        Delete a document and all its dependencies (session links, messages)
        ...
        """
        try:
            # Delete session-document relationships
            self.cursor.execute("""
                DELETE FROM session_documents 
                WHERE document_id = ?
            """, (document_id,))
            
            # Delete all messages from sessions that only had this document
            self.cursor.execute("""
            ...
            # Delete sessions that have no more documents
            self.cursor.execute("""
            ...
            self.conn.commit()
            logger.info(f"Successfully deleted document {document_id[:8]}... and dependencies")
            return True
            
        except sqlite3.Error as e:
            logger.error(f"Error deleting document {document_id[:8]}...: {e}")
            self.conn.rollback()
            return False

    def cleanup_old_documents(self, keep_count: int = 4) -> int:
        """
        Remove old documents to maintain a maximum count
        
        Args:
            keep_count: Maximum number of documents to keep (default: 4)
        ...
        try:
            ...
            if total_docs <= keep_count:
                logger.info(f"Document count ({total_docs}) is within limit ({keep_count}), no cleanup needed")
                return 0
            
            docs_to_remove = total_docs - keep_count
            logger.info(f"Cleaning up {docs_to_remove} old documents (keeping {keep_count} of {total_docs})")
            
            # Get oldest documents to remove
            oldest_docs = self.get_oldest_documents(docs_to_remove)
            
            if not oldest_docs:
                logger.warning("No documents found to remove")
                return 0
            ...
            for doc in oldest_docs:
                ...
                # Delete the document and its dependencies
                if self.delete_document_and_dependencies(doc_id):
                    removed_count += 1
                    logger.info(f"Removed document: {doc['filename']} (ID: {doc_id[:8]}..., Collection: {collection_name})")
                else:
                    logger.error(f"Failed to remove document: {doc_id[:8]}...")
            
            logger.info(f"Cleanup completed. Removed {removed_count} documents")
