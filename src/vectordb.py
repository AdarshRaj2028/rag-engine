...
class VectorDB:
    ...
    def delete_collection_by_name(self, collection_name: str) -> bool:
        """
        Delete a specific collection from ChromaDB by name.
    ...
    def get_all_collections(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            ...
            Each dict contains: name, count, metadata
        """
        try:
            collections = self.client.list_collections()
            collection_info = []
            ...
            logger.info(f"Retrieved information for {len(collection_info)} collections")
    ...
    def cleanup_orphaned_collections(self, valid_collection_names: List[str]) -> int:
        """
        Remove collections that are not in the list of valid collection names.
        
        Args:
            valid_collection_names: List of collection names that should be kept
        
        Returns:
            int: Number of orphaned collections removed
        """
        try:
            ...
            for collection_info in all_collections:
                ...
                # Skip if this collection should be kept
                if collection_name in valid_collection_names:
                    continue
                
                # Delete orphaned collection
                if self.delete_collection_by_name(collection_name):
                    orphaned_count += 1
                    logger.info(f"Removed orphaned collection: {collection_name} (contains {collection_info['count']} items)")
                else:
                    logger.error(f"Failed to remove orphaned collection: {collection_name}")
            
            if orphaned_count > 0:
                logger.info(f"Cleanup completed. Removed {orphaned_count} orphaned collections")
            else:
